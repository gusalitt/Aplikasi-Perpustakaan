<?php
  if (isset($_GET['action']) && $_GET['action'] == 'print_peminjaman') {
    $data_peminjaman = $_SESSION['print_peminjaman'] ?? [];
    $no = 1;
?>
  <style>
    @media print {
  ...
  <div id="printTable">
    <h1 style="text-align: center !important; margin: 1rem 0;">Data Peminjaman Buku App Perpustakaan</h1>
  ...
  <script>
    ...
    function printPeminjaman() {
      window.print();
      setTimeout(() => {
        window.location.href = "?page=peminjaman_buku";
      }, 1000);
    }
  </script>
